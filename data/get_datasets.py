...
from data.mnist import get_mnist_datasets,get_emnist_datasets,get_fashionMnist_datasets
from data.stl10 import get_stl10_datasets
...
from data.wikiart import get_wikiart_datasets
...
get_dataset_funcs = {
    ...
    'Fashion_mnist':get_fashionMnist_datasets,
    'stl10': get_stl10_datasets,
    'imagenet_100': get_imagenet_100_datasets,
    'herbarium_19': get_herbarium_datasets,
    'cub': get_cub_datasets,
    'aircraft': get_aircraft_datasets,
    'scars': get_scars_datasets,
    'wikiart': get_wikiart_datasets,
}
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    train_labelled_samples = deepcopy(datasets['train_labelled'])
    train_labelled_samples.transform = test_transform
    train_unlabelled_samples = deepcopy(datasets['train_unlabelled'])
    train_unlabelled_samples.transform = test_transform
    datasets['test_train'] = MergedDataset(labelled_dataset=train_labelled_samples,
                                  unlabelled_dataset=train_unlabelled_samples)
...
def get_class_splits(args):
    ...
    # GET CLASS SPLITS
    # -------------
    if args.dataset_name in ('cifar10', 'Fashion_mnist','stl10'):

        args.image_size = 32
        args.train_classes = range(5)
        args.unlabeled_classes = range(5, 10)
        args.spectral_dim  =15
        ...
        args.image_size = 32
        args.train_classes = range(15)
        args.unlabeled_classes = range(15, 26)
        args.spectral_dim  =30
    ...
    args.num_subspaces = len(args.train_classes)+len(args.unlabeled_classes)
    return args
