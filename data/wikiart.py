...
from config import wikiart_root
...
def get_wikiart_datasets(train_transform, test_transform, train_classes=range(3),
                           prop_train_labels=0.8, split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = ImageNetBase(root = wikiart_root, transform=train_transform)
    # Reset dataset
    ...
    test_dataset = ImageNetBase(root=wikiart_root, transform=test_transform)
...
if __name__ == '__main__':
    ...
    x = get_wikiart_datasets(None, None, split_train_val=False,
